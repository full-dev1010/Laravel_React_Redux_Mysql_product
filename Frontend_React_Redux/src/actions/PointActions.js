...
export function fetchPoints(product_id) {
  ...
  export function createPoint(pointData) {
    pointData.count = parseInt(pointData.count);
    pointData.price = parseInt(pointData.price);
    console.log("CGI createpoint action", pointData);
    return dispatch => {
      dispatch(fetchCreatePointBegin());
      return axios.post(serverURL + "/point", pointData)
        .then(res => res.data)
        .then(data => {
          console.log("CGI Createpoint success", data);
          dispatch(fetchCreatePointSuccess(data));
          return remakePoints(data);
        })
